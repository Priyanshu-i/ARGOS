...
export async function generateHuggingFaceResponse(messages: Message[], config: LLMConfig): Promise<string> {
  try {
    ...
    const response = await fetch(`${endpoint}${modelName}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${config.apiKey}`,
...
export async function generateResponse(messages: Message[], config: LLMConfig): Promise<string> {
  ...
  if (!provider && config.model.includes("/")) {
    provider = config.model.split("/")[0] as any;
  }
...
export function parseResponse(llmResponse: string): { visibleResponse: string; hiddenMessage: string | null } {
  // Looking for internal note pattern
  const internalNotePattern = /\[INTERNAL NOTE:([^\]]+)\]/i;
  const match = llmResponse.match(internalNotePattern);

  if (match) {
    // Extract the internal note
    const hiddenMessage = match[1].trim();
    
    // Remove the internal note from the response
    const visibleResponse = llmResponse.replace(internalNotePattern, "").trim();
...
// Continuing the prepareCustomerPrompt function...
